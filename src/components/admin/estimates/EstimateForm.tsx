...
import { LineItemRow } from "../invoices/LineItemRow";
import { SignaturePad } from "../invoices/SignaturePad";
import type { EstimatePreviewData } from "./EstimatePreview";
...
type EstimateFormProps = {
  ...
  onPreviewChange: (data: EstimatePreviewData) => void;
  onSaveDraft: (data: EstimateFormData) => Promise<void>;
  onSaveAndSend: (data: EstimateFormData) => Promise<void>;
  isSaving: boolean;
...
export function EstimateForm({
  ...
  const [estimateDate, setEstimateDate] = useState(
    initialData?.estimateDate ?? getToday()
  );
  const [validUntil, setValidUntil] = useState(
    initialData?.validUntil ?? getDefaultValidUntil()
  );
  ...
  useEffect(() => {
    onPreviewChange({
      estimateNumber: autoGenerate ? "Auto" : estimateNumber,
      clientName,
      clientAddress,
      estimateDate,
      validUntil,
  ...
  function getFormData(): EstimateFormData {
    return {
      estimateNumber,
      autoGenerate,
      clientName,
  ...
  function validate(): string | null {
    if (!autoGenerate && !estimateNumber.trim())
      return "Estimate number is required (or switch to auto-generate)";
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {autoGenerate ? (
          <p className="text-xs text-[var(--text-muted)]">
            A number will be assigned automatically when saved (e.g. EST-202603-001)
          </p>
        ) : (
          <input
            type="text"
      ...
      <section className="bg-[var(--neutral-lightest-gray)] rounded-lg p-4 space-y-2">
        ...
        <div className="flex justify-between text-base font-bold border-t border-[var(--border-light)] pt-2">
          <span className="text-[var(--primary-main)]">Estimated Total</span>
